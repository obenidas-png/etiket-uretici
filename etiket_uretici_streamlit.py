...
def parse_csv(df):
    ...
    for idx, row in df.iterrows():
        ...
        # Özellikler sütununu parse et - virgül içeren değerleri koru
        props = {}
        if pd.notna(row.get('Özellikler')):
            properties_str = str(row['Özellikler'])
            
            # Regex ile Ad: ve Değer: çiftlerini bul
            # Pattern: Ad:xxx,Değer:yyy (yyy sonraki Ad: ya kadar veya string sonuna kadar devam eder)
            pattern = r'Ad:([^,]+),Değer:([^,]+(?:,[^Ad][^:]*)*?)(?=,Ad:|$)'
...
def draw_label(c, x, y, width, height, data):
    ...
    if 'cerasus' in store:
        ...
        # Chepniq ve diğer mağazalar için lazer etiketi (7 satır - Lazer 2 satır)
        line_height = height / 7
        for i in range(1, 7):
            line_y = y + (i * line_height)
            c.setLineWidth(0.3)
        ...
        rows = [
            ...
            ('', pers_text_line2)         # 2. satır (label boş)
        ]
